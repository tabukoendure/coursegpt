...
export default function DashboardLayout() {
  ...
  const notifications = [
    { id: 1, text: "New past questions added for BCH201", time: "2h ago" },
    { id: 2, text: "Your study plan for ANA204 is ready", time: "5h ago" }
  ];
    ...
    <div className="min-h-screen bg-bg flex flex-col md:flex-row">
      ...
      <div className="md:hidden bg-white border-b border-border px-4 py-3 flex justify-between items-center sticky top-0 z-40">
        <div className="flex items-center space-x-2">
          ...
          <span className="text-lg font-black text-text-primary tracking-tighter uppercase">COURSE<span className="text-primary">GPT</span></span>
        </div>
      ...
      <aside className="w-64 border-r border-border bg-white hidden md:flex flex-col h-screen sticky top-0">
        ...
        <nav className="flex-1 px-4 space-y-8 overflow-y-auto pb-10">
          <div>
            <p className="px-4 text-[10px] uppercase font-black text-text-secondary tracking-widest mb-4 opacity-40">Main Navigation</p>
          ...
          <div>
            <p className="px-4 text-[10px] uppercase font-black text-text-secondary tracking-widest mb-4 opacity-40">Your Account</p>
            <div className="space-y-1">
              {accountItems.map((item) => (
                <Link
                  ...
                  to={item.path}
                  className={`flex items-center space-x-3 px-4 py-3 rounded-2xl font-bold transition-all ${
                    location.pathname === item.path ? 'bg-primary-light text-primary' : 'text-text-secondary hover:bg-bg'
                  }`}
                >
        ...
        <div className="p-6 border-t border-border bg-bg/5">
          <div className="flex items-center space-x-3">
            ...
            <div className="overflow-hidden">
              ...
              <div className="text-[10px] text-text-secondary truncate font-bold">{profile.level} L • {profile.department}</div>
            </div>
          </div>
        </div>
      </aside>
        ...
        {navItems.map((item) => (
          <Link
            ...
            className={`flex flex-col items-center justify-center space-y-1 transition-all ${
              location.pathname === item.path ? 'text-primary' : 'text-text-secondary opacity-60'
            }`}
          >
